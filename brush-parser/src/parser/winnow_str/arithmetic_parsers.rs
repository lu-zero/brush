//! Arithmetic parsers module
//!
//! Contains parsers for arithmetic expressions and commands (Tier 15-16)
...
use crate::parser::winnow_str::{compound_parsers, control_flow_parsers, context::ParseContext, types::PError};
...
pub fn normalize_arithmetic_expr(s: &str) -> String {
...
pub fn arithmetic_expression<'a>() -> impl Parser<StrStream<'a>, ast::UnexpandedArithmeticExpr, PError>
{
...
pub fn arithmetic_for_body<'a>(
    ...
    move |input: &mut StrStream<'a>| {
        winnow::combinator::alt((
            ...
            winnow::combinator::preceded(compound_parsers::sequential_sep(), control_flow_parsers::do_group(ctx, tracker)),
            // Try do_group directly (for " do" - spaces consumed by keyword)
            control_flow_parsers::do_group(ctx, tracker),
            // Try brace_group (convert to DoGroupCommand)
            compound_parsers::brace_group(ctx, tracker).map(|bg| ast::DoGroupCommand {
                list: bg.list,
                loc: bg.loc,
            }),
        ))
...
pub fn for_or_arithmetic_for<'a>(
    ...
    move |input: &mut StrStream<'a>| {
        ...
        if ctx.options.posix_mode || ctx.options.sh_mode {
            control_flow_parsers::for_clause(ctx, tracker)
            ...
            winnow::combinator::alt((
                ...
                control_flow_parsers::for_clause(ctx, tracker).map(ast::CompoundCommand::ForClause),
            ))
            .parse_next(input)
        }
    }
