...
//!
//! Contains parsers for pipeline-related functions (Tier 4)
...
pub fn add_pipe_extension_redirect(cmd: &mut ast::Command) {
...
pub fn parse_trailing_command(
    ...
    super::command(&ctx, &tracker).parse_next(&mut stream).ok()
}
...
pub fn pipe_sequence<'a>(
    ...
    move |input: &mut StrStream<'a>| {
        let (first, rest) = (
            super::command(ctx, tracker),
            repeat::<_, _, Vec<_>, _, _>(
                0..,
                (
                    winnow::combinator::preceded(super::char_parsers::spaces(), pipe_operator()), // spaces then |
                    winnow::combinator::preceded(
                        (
                            super::line_parsers::linebreak(),
...
pub fn pipeline_timed<'a>(
    ...
    move |input: &mut StrStream<'a>| {
        ...
        if super::keyword("time").parse_next(input).is_err() {
            return Ok(None);
        }
        ...
        let has_posix_flag = winnow::combinator::opt(winnow::combinator::terminated(
            "-p",
            super::char_parsers::spaces(),
        ))
        .parse_next(input)?
...
pub fn pipeline_bang<'a>() -> impl Parser<StrStream<'a>, usize, PError> {
    winnow::combinator::repeat(
        0..,
        winnow::combinator::terminated(super::keyword("!"), super::char_parsers::spaces()),
    )
