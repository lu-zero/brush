...
mod context;
mod extended_test_parsers;
mod function_parsers;
mod line_parsers;
mod pipeline_parsers;
mod program_parsers;
mod redirection_parsers;
mod types;
mod word_parsers;
...
// Import types that are used throughout the file
use crate::parser::winnow_str::command_parsers::assignment_word;
use crate::parser::winnow_str::redirection_parsers::here_documents;
use crate::parser::winnow_str::types::{PError, StrStream};

// Re-export public items from submodules
pub use char_parsers::{
    array_spaces, comment, extglob_pattern, newline, parse_balanced_delimiters, peek_char,
    peek_op2, peek_op3, spaces, spaces1, tilde_expansion,
};
pub use command_parsers::cmd_prefix;
...
pub use word_parsers::{
    ...
    double_quoted_string, escape_sequence, is_reserved_word, non_reserved_word, simple_variable,
    single_quoted_string, special_parameter, word_as_ast, word_part, wordlist,
};

pub use redirection_parsers::{IoRedirectResult, io_number, io_redirect, redirect_list};
    ...
    move |input: &mut StrStream<'a>| {
        (
            ...
            winnow::combinator::preceded(
                (linebreak(), spaces()),
                pipeline_parsers::pipeline(ctx, tracker),
            ), // optional newlines+spaces, then pipeline
        )
